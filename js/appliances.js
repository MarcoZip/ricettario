...
export const APPLIANCE_KB = [
  ...
  },
  {
    ...
    sequence: "Controlla che il display mostri l'ora (se lampeggia, il forno non scalda) · ruota la manopola di SINISTRA sul simbolo della funzione (in alto c'è lo spegnimento, in alto a destra la lampadina della luce, in basso a sinistra l'asterisco dello scongelamento) · ruota quella di DESTRA sulla temperatura, fra 50 e 275 °C: il forno parte da solo, non c'è un tasto avvio · per la durata premi il tasto centrale con l'orologio finché lampeggia la spia giusta, poi regola con − e + · a fine cottura riporta ENTRAMBE le manopole su OFF.",
    levels: 5,
    racks: [
    ...
    howto: [
      ...
      "IMPORTANTE: sulla manopola delle funzioni NON ci sono scritte, solo SIMBOLI. Indica quindi la funzione descrivendo il simbolo, non un nome: ventola dentro un cerchio = cottura ventilata; ventola senza calore = scongelamento; barra in alto = grill; barra più larga in alto = grill grande; barra in alto insieme alla ventola = grill ventilato (arrosti e gratin); lampadina = luce del forno.",
      "POSIZIONI VERIFICATE sulla foto: la manopola ha circa 7 posizioni più lo spegnimento (quadratino in alto). La LAMPADINA (luce) è in alto a destra; l'ASTERISCO/fiocco (scongelamento) è in basso a sinistra. Le posizioni degli altri simboli non sono confermate: se serve, invita a riconoscerli dal disegno.",
